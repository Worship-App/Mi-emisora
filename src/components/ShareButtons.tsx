...
export default function ShareButtons({ song, className = '' }: ShareButtonsProps) {
  const [showOptions, setShowOptions] = useState(false)
  const [copied, setCopied] = useState(false)

  const shareUrl = typeof window !== 'undefined' 
    ? `${window.location.origin}?song=${song.id}`
    : `https://miemisoraweb.com?song=${song.id}`

  const shareText = `🎵 Escucha "${song.title}" de ${song.artist} en Mi Emisora Web!`
  ...
  return (
    <div className={`relative ${className}`}>
      <button
        ...
        className="p-2 rounded-full text-gray-400 hover:text-gold-400 hover:bg-white/10 transition-colors"
      ...
      {showOptions && (
        <>
          ...
          {/* Share Menu */}
          <div className="absolute right-0 bottom-full mb-2 w-56 glass-premium-strong rounded-2xl p-3 z-50 border border-gold-500/20 shadow-2xl">
